...
#define ENTITY_H_INCLUDED
    ...
    class Entity //modele de toutes les entités
    {
        public:
            ...
            bool collision(Entity *other); //renvoit vrai si il y a collision avec l'autre entité
            bool get_parameter(int wich_paremeter); //retourne le paramètre demandé

            Hitbox* get_hitbox(); //retourne la hitbox de l'entité

            void set_pos(int x, int y); //change la postion du CENTRE de l'entité
            void set_parameter(int wich_paremeter, bool enable); //permet de changer un paramètre
        ...
        protected:
            ...
            bool m_parameters[7]; /*contient plusieurs paramètres qui sont:
